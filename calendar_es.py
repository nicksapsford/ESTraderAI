"""
ESTrader AI -- calendar_es.py  (Guinevere)
...
log = logging.getLogger("ESTrader.Guinevere")
...
def check_calendar(now_utc: Optional[datetime] = None) -> dict:
    ...
    mins_to_close = 0
    try:
        from data_feed_es import get_session_phase
        if get_session_phase(now_utc) == "US_SESSION":
            session_active = True
            close_dt = now_utc.replace(hour=20, minute=55, second=0, microsecond=0)
...
if __name__ == "__main__":
    ...
    log.info("Guinevere self-test (ESTrader AI -- US500)")
