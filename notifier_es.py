"""
ESTrader AI -- notifier_es.py  (Percival)
...
log = logging.getLogger("ESTrader.Percival")
...
def notify_trade_opened(
    ...
    _send(
        title   = "[US500] Trade Opened -- ESTrader AI",
        message = (
...
def notify_trade_closed_win(
    ...
    _send(
        title   = "[US500] Trade WON -- ESTrader AI",
        message = (
...
def notify_trade_closed_loss(
    ...
    _send(
        title   = "[US500] Trade Lost -- ESTrader AI",
        message = (
            f"{direction} closed at {exit_price:,.1f}\n"
...
def notify_kill_switch_triggered(
    ...
    _send(
        title   = f"[US500] KILL SWITCH Tier {tier} -- ESTrader AI",
        message = (
            f"{reason}\n"
...
def notify_kill_switch_reset(tier: int, wait_hours: int, capital: float) -> None:
    _send(
        title   = "[US500] Trading Resuming -- ESTrader AI",
        message = (
...
def notify_tier3_urgent() -> None:
    _send(
        ...
        message = (
            "ESTrader AI kill switch triggered 3x in 48 hours.\n"
...
def notify_system_startup(capital: float, mode: str = "PAPER") -> None:
    _send(
        title   = f"[US500] ESTrader AI Started ({mode})",
        message = (
            f"ESTrader AI is live.\n"
...
def notify_system_shutdown(capital: float) -> None:
    _send(
        title   = "[US500] ESTrader AI Shutdown",
        message = (
            f"ESTrader AI stopped cleanly.\n"
...
def notify_memory_pattern(pattern: str) -> None:
    """Percival: fired once when a Merlin's Memory pattern first reaches n=3 (ESTrader
    pilot). Informational -- helps Nick/Archie see when memory starts learning."""
    _send(
        title   = "[ESTrader] Memory pattern detected",
        message = f"Merlin's Memory reached n=3 on a new pattern:\n{pattern}",
    )
...
def notify_system_error(error_msg: str) -> None:
    _send(
        title   = "[US500] System Error -- ESTrader AI",
