"""
ESTrader AI -- backtest_es.py  (Backtest Bench)

S&P 500 (US500) spread betting backtest engine. Validates the LIVE entry gate
(Lancelot / pre_checks_es) against real Yahoo Finance history and specifically
...
Market    : S&P 500, Yahoo ticker ^GSPC (ESTrader's live Yahoo fallback US_TICKER)

US500 mechanics (matched to strategy_es / pre_checks_es):
...
Output:
  ...
  logs/backtest_es_trades.csv   -- every trade across both configs
"""
...
log = logging.getLogger("ESTrader.Backtest")

# ── Constants (matched to live strategy_es / pre_checks_es) ────────────────────
...
US_TICKER = "^GSPC"              # Yahoo Finance S&P 500 (ESTrader live fallback)

# Live entry-gate momentum / choppiness thresholds (mirror pre_checks_es)
...
# ── Indicator functions (verbatim from data_feed_es / the FTSE template) ───────
...
# ── LIVE entry gate (faithful port of pre_checks_es quality checks) ────────────
#
...
# ── Spread-bet trade record (mirror strategy_es.USTrade) ───────────────────────
...
def build_comparison_lines(baseline: dict, relaxed: dict,
    ...
    L = []
    L.append(sep)
    L.append("  ESTRADER AI -- S&P 500 BACKTEST  |  1h RSI VETO: BASELINE vs RELAXED")
...
def save_trades_csv(all_runs: list) -> None:
    path = LOGS_DIR / "backtest_es_trades.csv"
...
def run_backtest() -> None:
    ...
    log.info("ESTrader AI -- S&P 500 Backtest (BASELINE vs RELAXED 1h RSI veto)")
    ...
    log.info("  logs/backtest_es_trades.csv")
