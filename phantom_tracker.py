...
ETH loops sharing one file) cannot corrupt es_phantom_trades.csv.
"""
...
# Every timestamp this module reads or writes (es_phantom_trades.csv rows, log
...
PHANTOM_CSV = os.path.join(os.path.dirname(__file__), 'logs', 'es_phantom_trades.csv')
...
FIELDNAMES = _BASE_FIELDS + INDICATOR_COLUMNS + NEW_HORIZON_COLUMNS + BENCHMARK_COLUMNS + DEDUP_COLUMNS
...
# System 5 Review desk-wide (18 Jul 2026): 10 -> 8.
# Job 5 recalibration (24 Jul 2026): re-verified, HELD at 8. Fresh median |1hr move| over 45
# phantom rows is 7.9pt; threshold 8 (~median) already classifies ~49%% (CORRECT+WRONG),
# squarely in the 40-60%% target, so no numeric change was warranted. Window kept at 1hr: the
# 2hr median (8.1pt) is no more decisive than 1hr and the 1hr->2hr flip rate did not justify a
# switch. Existing rows re-scored on 24 Jul (idempotent at 8; see phantom_rescore_20260724.txt).
...
def _migrate_header_if_needed():
    """One-time in-place upgrade: if es_phantom_trades.csv exists with an older
    ...
    with _csv_lock:
        ...
        logger.info("phantom_tracker: migrated es_phantom_trades.csv header "
...
def _ensure_csv_exists():
    ...
    if not os.path.exists(PHANTOM_CSV):
        with open(PHANTOM_CSV, 'w', newline='') as f:
            ...
            writer.writeheader()
        logger.info("phantom_tracker: Created es_phantom_trades.csv")
        return
...
def resolve_stale_pending(get_historical_price_fn):
    ...
    Scans es_phantom_trades.csv for PENDING rows older than 2 hours and resolves
