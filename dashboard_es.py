"""
ESTrader AI -- dashboard_es.py
Two-page browser dashboard at http://localhost:5007
...
TRADES_LOG       = LOG_DIR / "es_trades.csv"
...
def get_stay_out_quality():
    """Read logs/es_phantom_trades.csv (built by Job 2). Degrade gracefully.

    Job 6: quality metrics default to UNIQUE signals only (deduplicated !=
    'TRUE'); the ALL-DECISIONS set is also returned so the phantom-page toggle
    can switch client-side with no refetch. The "last 50" window is applied
    AFTER filtering to unique. Backward-compat: top-level keys mirror the
    UNIQUE (default) view."""
    # ALBION RULE: es_phantom_trades.csv timestamps are UTC — never BST/local.
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logs', 'es_phantom_trades.csv')
...
<title>ESTrader A.I. &mdash; S&amp;P 500</title>
<style>
:root{
...
.phase-ASIAN     {background:rgba(155,89,182,0.15);color:#9b59b6;}
.phase-US_SESSION{background:rgba(52,152,219,0.15);color:#3498db;}
.phase-CLOSED    {background:rgba(230,126,34,0.15);color:#e67e22;}
...
<div id="page1" class="page-wrap">
  <div class="main" id="main-grid">
    <div style="grid-column:1/-1;color:var(--muted);padding:40px;text-align:center">Loading ESTrader AI...</div>
  </div>
</div>
...
var PHASE_BOUNDS = [
  {name:'ASIAN',      end:7*60 + 0},     /* overnight until 07:00 */
  {name:'PRE_MARKET', end:13*60 + 30},   /* 07:00-13:30 */
  {name:'US_SESSION', end:21*60 + 0},    /* 13:30-21:00 (prime) */
...
function confirmShutdown(){
  fetch('/api/shutdown', {method:'POST'})
    .then(function(){
      document.body.innerHTML = '<div style="display:flex;height:100vh;align-items:center;justify-content:center;background:#0d0d0d;color:#3a3ad6;font-family:monospace;font-size:18px;">ESTrader AI shut down. You may close this window.</div>';
    })
...
/* Merlin's Memory card (Page 1, compact) -- ESTrader has NO Morgan and NO Guinevere;
   this pilot tests Arthur's episodic memory instead. Shows the latest memory build. */
function renderMemoryCard(mem, phase){
  mem = mem || {};
  var trades   = mem.trades   || 0;
  var phantoms = mem.phantoms || 0;
  var patterns = mem.patterns || 0;
  var tokens   = mem.tokens   || 0;
  var pct = Math.min(100, Math.round(tokens / 600 * 100));
  var ph = phase || '--';
  var phCol = ph === 'US_SESSION' ? 'bull' : (ph === 'CLOSED' ? 'bear' : 'neut');
  return '<div class="card"><div class="card-title gold">Merlin&#39;s Memory</div>' +
    '<div style="display:flex;gap:14px;font-size:11px;margin-bottom:5px;">' +
      '<span>Trades: <strong style="color:var(--gold-lite)">' + trades + '</strong></span>' +
      '<span>Phantoms: <strong style="color:var(--gold-lite)">' + phantoms + '</strong></span>' +
      '<span>Patterns: <strong style="color:var(--text)">' + patterns + '</strong></span>' +
    '</div>' +
    '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;">' +
      '<span style="font-size:10px;color:var(--muted);min-width:60px">Tokens</span>' +
      '<div class="score-bar"><div class="score-fill score-med" style="width:' + pct + '%"></div></div>' +
      '<span style="font-size:11px;font-weight:700;min-width:70px;text-align:right">' + tokens + '/600</span></div>' +
    '<div style="font-size:11px;color:var(--muted);">Session: <strong class="' + phCol + '">' + ph +
      '</strong> &mdash; no Morgan, no Guinevere (memory pilot)</div>' +
    '</div>';
}
...
function renderRightPanel(d){
  ...
  var calHTML = '<div class="card" style="flex-shrink:0"><div class="card-title gold">US Calendar (Lancelot event block)</div>' +
...
function renderPage1(d){
  ...
  var centreCol = '<div class="col">' +
    ...
    reasonBox + warnHTML +
    '</div>' +
    renderMemoryCard(d.memory || {}, d.phase) +
...
function renderPage2(d){
  ...
  if(total === 0){
    ...
    if(sesKeys.length > 0){
      var sesLabel = {'ASIAN':'Asian','PRE_MARKET':'Pre-Market','US_SESSION':'US Session','CLOSED':'Closed'};
    ...
    // ESTrader: no Morgan warning/critical banners -- Morgan is removed by design.
  }
...
def api_archie_brief():
    ...
    try:
        ...
        state = get_state()
    txt = archie_brief.build_system_brief(state, "ESTrader", "S&P 500", str(LOG_DIR))
...
# /api/reset-morgan REMOVED on ESTrader -- Morgan is excluded by design (memory pilot).
...
if __name__ == "__main__":
    ...
    log.info("ESTrader AI Dashboard starting on http://localhost:%d", PORT)
