...
log = logging.getLogger("ESTrader.Strategy")
...
FORCE_CLOSE_MIN        = 55      # UTC -- 20:55 UTC, before the 21:00-22:00 maintenance break
...
def should_force_close(ts_utc=None) -> bool:
    """Return True only in the 20:55-21:00 UTC window -- force close positions before the
    daily maintenance break. Bounded so it never fires during the 22:00 ASIAN reopen
    (ESTrader trades ~23h, unlike the NYSE-only original)."""
    if ts_utc is None:
        ts_utc = datetime.now(timezone.utc)
    ts_utc = ts_utc.astimezone(timezone.utc)
    return ts_utc.hour == FORCE_CLOSE_HOUR and ts_utc.minute >= FORCE_CLOSE_MIN
