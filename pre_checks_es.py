"""
ESTrader AI -- pre_checks_es.py  (Lancelot)
...
from data_feed_es import get_session_phase, ASIAN, PRE_MARKET, US_SESSION, CLOSED

log = logging.getLogger("ESTrader.Lancelot")
...
# Session time boundaries (minute-of-day, UTC). ESTrader trades ~23h -- NOT NYSE-only.
_US_OPEN       = 13 * 60 + 30    # 13:30 -- US cash open (summer/EDT per ESTrader spec)
_MARKET_CLOSE  = 21 * 60         # 21:00 -- US cash close / maintenance begins
_NO_NEW_ENTRY  = 20 * 60 + 55    # 20:55 -- too close to the maintenance break
_US_OPEN_END   = 13 * 60 + 45    # 13:45 -- opening-volatility window ends
...
def check_market_open(ts_utc: Optional[datetime] = None) -> dict:
    """Allow entries whenever the market is open -- any phase except CLOSED. ESTrader
    trades the US500 CFD ~23h/day (ASIAN / PRE_MARKET / US_SESSION); it is NOT
    restricted to NYSE cash hours. CLOSED = 21:00-22:00 maintenance + the weekend."""
    if get_session_phase(ts_utc) != CLOSED:
        return _pass()
    return _fail("Market closed -- daily maintenance (21:00-22:00 UTC) or weekend.")


def check_near_close(ts_utc: Optional[datetime] = None) -> dict:
    """Block new entries in the 20:55-21:00 UTC window -- force close before the daily
    maintenance break is imminent. Bounded so it never blocks the 22:00 ASIAN reopen."""
    t = _now_minute(ts_utc)
    if _NO_NEW_ENTRY <= t < _MARKET_CLOSE:
        return _fail("Too close to maintenance -- no new entries 20:55-21:00 UTC (force close pending).")
    return _pass()


def check_us_open_volatility(ts_utc: Optional[datetime] = None) -> dict:
    """Block the first 15 minutes after the US cash open (13:30-13:45 UTC) -- opening
    volatility. Other phases (PRE_MARKET / ASIAN) are unaffected."""
    t = _now_minute(ts_utc)
    if _US_OPEN <= t < _US_OPEN_END:
        return _fail("US open volatility -- no entries in first 15 min (13:30-13:45 UTC).")
    return _pass()
...
def run_all_pre_checks(
    ...
    SKIP only the three SOFT quality checks -- SSL agreement, Momentum strong, Candle
    ...
    losses / cooldown / market open / near close / US open volatility) run FIRST and are
    NEVER relaxed, and the Daily trend filter, 1h RSI and Not-choppy (volatility) quality
    checks are ALSO still enforced -- so no risk control can ever be bypassed. Arthur still
    needs >= 60 confidence to act (enforced in main).
    """
    log.info("--- Lancelot running pre-checks ---")
    _RELAXABLE = {"SSL agreement", "Momentum strong", "Candle confirmed"}
