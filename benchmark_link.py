...
All times UTC. Benchmark port = original port + 20 (USBench 5024).
"""
import json
import logging
...
# Matched benchmark's state endpoint (localhost). ESTrader trades US500 -- the SAME
# instrument as USTrader -- so USBenchmark (5024) is the appropriate FAIR-comparison
# control for now (Phantom/Benchmark Link brief, Part 4). SWITCH this to an ESBenchmark
# port if/when one is built (ESBenchmark is deliberately NOT built yet -- see brief Part 5).
BENCHMARK_STATE_URL = "http://localhost:5024/api/state"
TIMEOUT_SECONDS = 1.5
