...
def build_system_brief(state, system_name, asset_label, logs_dir=None, now_utc=None):
    ...
    if isinstance(ct, dict) and ct:
        ...
        a("No open position")
    a("")
    # MORGAN removed on ESTrader by design. MERLIN'S MEMORY section is added below.
    a("MERLIN'S MEMORY")
    try:
        import merlin_memory_es
        _mstats = merlin_memory_es.get_memory_stats()
        a("Trades in memory: %s | Phantoms in memory: %s" % (_mstats.get("trades", 0), _mstats.get("phantoms", 0)))
        a("Patterns active: %s | Memory tokens: %s/600" % (_mstats.get("patterns", 0), _mstats.get("tokens", 0)))
    except Exception as _mexc:
        a("Memory stats unavailable: %s" % _mexc)
    a("")
    a("STAY OUT QUALITY")
    ...
    a("")
    # Guinevere REMOVED on ESTrader by design. The US economic-calendar HARD BLOCK is
    # KEPT (as a Lancelot risk control) and its status shows under LANCELOT / MARKET.
    a("US CALENDAR (Lancelot event block)")
    a("  %s" % (state.get("calendar") or "no calendar data"))
    a("")
